...
using cubey::ConfigOptionStability;
...
constexpr std::array<std::string_view, 9> kCloudDensityModels{
    "surface-volume",   "experimental-aerial-orbit",
    "ref-density",      "cloud-ref-compatible",
...
constexpr std::array<std::string_view, 14> kCloudWeatherPresets{
...
constexpr std::string_view kCloudGeneralGroup = "Clouds/General";
constexpr std::string_view kCloudSamplingGroup = "Clouds/Sampling";
constexpr std::string_view kCloudLayerGroup = "Clouds/Layer";
constexpr std::string_view kCloudShapeGroup = "Clouds/Shape";
constexpr std::string_view kCloudLightingGroup = "Clouds/Lighting";
constexpr std::string_view kCloudReferenceGroup = "Clouds/Reference Diagnostics";
constexpr std::string_view kCloudAerialOrbitGroup = "Clouds/Deferred Aerial Orbit";
...
constexpr std::array<ConfigOptionDescriptor, 302> kRunConfigOptions{
    ...
    option(RunConfigOptionId::OceanMeshCells, "ocean.mesh_cells", "--ocean-mesh-cells",
           "Mesh Cells", "Ocean", "Maximum grid resolution per ocean clipmap patch.",
           ConfigOptionType::UInt32, bounded_range(32.0, 512.0)),
    option(RunConfigOptionId::OceanMeshLodLevels, "ocean.mesh_lod_levels",
           "--ocean-mesh-lod-levels", "Mesh LOD Levels", "Ocean",
           "Minimum number of concentric ocean clipmap levels.", ConfigOptionType::UInt32,
           bounded_range(1.0, 6.0)),
    option(RunConfigOptionId::OceanHorizonTargetNearCell, "ocean.horizon_target_near_cell_m",
    ...
    option(RunConfigOptionId::OceanSelfShadowSteps, "ocean.self_shadow_steps",
    ...
    option(RunConfigOptionId::OceanDetailAntiRepeatStrength,
           "ocean.detail_anti_repeat_strength", "--ocean-detail-anti-repeat-strength",
           "Detail Anti Repeat", "Ocean",
           "Strength of far-field normal and foam domain perturbation.", ConfigOptionType::Float,
           bounded_range(0.0, 1.0)),
    option(RunConfigOptionId::OceanDetailFilter, "ocean.detail_filter",
    ...
    option(RunConfigOptionId::OceanSizeReference, "ocean.size_reference",
           ...
           ConfigOptionType::Bool, no_range(), {}, "--no-ocean-size-reference"),
    ...
    option(RunConfigOptionId::TerrainRecipe, "terrain.recipe", "--terrain-recipe", "Recipe",
           ...
           ConfigOptionType::String),
    option(RunConfigOptionId::TerrainStudyField, "terrain.study_field", "--terrain-study-field",
    ...
    option(RunConfigOptionId::TerrainPlacementIndex, "terrain.placement_index",
           "--terrain-placement-index", "Placement Index", "Terrain/Backdrop Stage",
           "Deterministic raw-sample placement index.", ConfigOptionType::UInt32),
    ...
    option(RunConfigOptionId::CloudEnabled, "clouds.enabled", "--clouds", "Clouds",
           kCloudGeneralGroup, "Enable cloud layer rendering in shared atmosphere-backed projects.",
           ConfigOptionType::Bool, no_range(), {}, "--no-clouds"),
    option(RunConfigOptionId::CloudDebugView, "clouds.debug_view", "--cloud-debug-view",
           "Debug View", kCloudReferenceGroup,
           "Cloud layer debug or diagnostic view; intended for inspection, not production configs.",
           ConfigOptionType::String, no_range(), {}, {}, ConfigOptionStability::Reference),
    option(RunConfigOptionId::CloudCameraMode, "clouds.camera_mode", "--cloud-camera-mode",
           "Camera Mode", kCloudReferenceGroup,
           "Reference/capture app initial camera mode; shared consumers usually own camera state.",
           ConfigOptionType::Enum, no_range(), enum_choices(kCloudCameraModes), {},
           ConfigOptionStability::Reference),
    option(RunConfigOptionId::CloudQuality, "clouds.quality", "--cloud-quality", "Quality",
           kCloudSamplingGroup, "Cloud render quality preset.", ConfigOptionType::Enum, no_range(),
           enum_choices(kCloudQualities)),
    option(RunConfigOptionId::CloudViewSteps, "clouds.view_steps", "--cloud-view-steps",
           "View Steps", kCloudSamplingGroup, "Cloud ray-march view steps override.",
    ...
    option(RunConfigOptionId::CloudViewSamples, "clouds.view_samples", "--cloud-view-samples",
           "View Samples", kCloudSamplingGroup, "Cloud ray-start samples per pixel.",
           ConfigOptionType::UInt32, bounded_range(1.0, 4.0)),
    option(RunConfigOptionId::CloudViewSampleMode, "clouds.view_sample_mode",
           "--cloud-view-sample-mode", "View Sample Mode", kCloudSamplingGroup,
    ...
    option(RunConfigOptionId::CloudDensityModel, "clouds.density_model", "--cloud-density-model",
           "Density Model", kCloudShapeGroup,
           "Cloud density model. surface-volume is Cloud V1; experimental-aerial-orbit and "
           "ref-density are deferred compatibility/reference paths.",
    ...
    option(RunConfigOptionId::CloudResolveMode, "clouds.resolve_mode", "--cloud-resolve-mode",
           "Resolve Mode", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudBackgroundMode, "clouds.background_mode",
           "--cloud-background-mode", "Background Mode", kCloudReferenceGroup,
           "Reference-only standalone cloud background mode: atmosphere or water-context.",
    ...
    option(RunConfigOptionId::CloudDistanceMode, "clouds.distance_mode", "--cloud-distance-mode",
           "Distance Mode", kCloudAerialOrbitGroup,
           "Cloud distance regime. auto enables the Cloud V1 surface horizon handoff; "
           "local is the reference fallback; orbit-shell and blend-debug are deferred.",
    ...
    option(RunConfigOptionId::CloudTopAltitude, "clouds.top_altitude_m", "--cloud-top-altitude-m",
           "Cloud Top", kCloudLayerGroup,
           "Cloud layer top altitude above the planet surface in meters.", ConfigOptionType::Float,
           min_range(0.0)),
    option(RunConfigOptionId::CloudCoverage, "clouds.coverage", "--cloud-coverage", "Coverage",
           kCloudLayerGroup, "Base cloud coverage fraction.", ConfigOptionType::Float,
           bounded_range(0.0, 1.0)),
    option(RunConfigOptionId::CloudDensity, "clouds.density", "--cloud-density", "Density",
           kCloudLayerGroup, "Cloud extinction density multiplier.", ConfigOptionType::Float,
           min_range(0.0)),
    option(RunConfigOptionId::CloudWeatherScale, "clouds.weather_scale_km",
           "--cloud-weather-scale-km", "Weather Scale", kCloudLayerGroup,
    ...
    option(RunConfigOptionId::CloudFootprintFilterStrength, "clouds.footprint_filter_strength",
           "--cloud-footprint-filter-strength", "Footprint Filter", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudEdgeSoftness, "clouds.edge_softness", "--cloud-edge-softness",
           "Edge Softness", kCloudShapeGroup, "Strength of footprint-aware density edge softening.",
           ConfigOptionType::Float, bounded_range(0.0, 2.0)),
    option(RunConfigOptionId::CloudEdgeDetailFade, "clouds.edge_detail_fade",
           "--cloud-edge-detail-fade", "Edge Detail Fade", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudEdgeResolveStrength, "clouds.edge_resolve_strength",
           "--cloud-edge-resolve-strength", "Edge Resolve", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudVerticalShearFraction, "clouds.vertical_shear_fraction",
           "--cloud-vertical-shear-fraction", "Vertical Shear", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudWindSpeed, "clouds.wind_speed_mps", "--cloud-wind-speed-mps",
           "Wind Speed", kCloudLayerGroup, "Cloud weather-map wind speed in meters per second.",
           ConfigOptionType::Float, min_range(0.0)),
    option(RunConfigOptionId::CloudShadowStrength, "clouds.shadow_strength",
           "--cloud-shadow-strength", "Shadow Strength", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudHorizonStrength, "clouds.horizon_strength",
           "--cloud-horizon-strength", "Horizon Strength", kCloudLightingGroup,
           "Strength of Cloud V1 surface horizon fill and glow.", ConfigOptionType::Float,
           bounded_range(0.0, 2.0)),
    option(RunConfigOptionId::CloudWeatherFronts, "clouds.weather_fronts", "--cloud-weather-fronts",
           "Weather Fronts", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudWeatherCells, "clouds.weather_cells", "--cloud-weather-cells",
           "Weather Cells", kCloudShapeGroup,
           "Feature-isolation weight for cellular cloud structures.", ConfigOptionType::Float,
           bounded_range(0.0, 1.0)),
    option(RunConfigOptionId::CloudWeatherStreaks, "clouds.weather_streaks",
           "--cloud-weather-streaks", "Weather Streaks", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudWeatherSoftness, "clouds.weather_softness",
           "--cloud-weather-softness", "Weather Softness", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudWeatherInfluence, "clouds.weather_influence",
           "--cloud-weather-influence", "Weather Influence", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudDetailErosion, "clouds.detail_erosion", "--cloud-detail-erosion",
           "Detail Erosion", kCloudShapeGroup,
    ...
    option(RunConfigOptionId::CloudAmbientStrength, "clouds.ambient_strength",
           "--cloud-ambient-strength", "Ambient Strength", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudDirectStrength, "clouds.direct_strength",
           "--cloud-direct-strength", "Direct Strength", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudPhaseStrength, "clouds.phase_strength", "--cloud-phase-strength",
           "Phase Strength", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudTwilightColorStrength, "clouds.twilight_color_strength",
           "--cloud-twilight-color-strength", "Twilight Color", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudTwilightEdgeStrength, "clouds.twilight_edge_strength",
           "--cloud-twilight-edge-strength", "Twilight Edge", kCloudLightingGroup,
           "Low-sun color boost for cloud optical edges and rim response.", ConfigOptionType::Float,
           bounded_range(0.0, 2.0)),
    option(RunConfigOptionId::CloudTwilightSaturationStrength,
           ...
           "Twilight Saturation", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudAfterglowStrength, "clouds.afterglow_strength",
           "--cloud-afterglow-strength", "Afterglow", kCloudLightingGroup,
           "Art-directed red, pink, or purple low-sun cloud edge accent.", ConfigOptionType::Float,
           bounded_range(0.0, 2.0)),
    ...
    option(RunConfigOptionId::CloudFinalSaturation, "clouds.final_saturation",
           "--cloud-final-saturation", "Final Saturation", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudResolveStrength, "clouds.resolve_strength",
           "--cloud-resolve-strength", "Resolve Strength", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudResolveRadius, "clouds.resolve_radius_px",
           "--cloud-resolve-radius-px", "Resolve Radius", kCloudLightingGroup,
           "cloud_ref post-resolve blur radius in pixels; shared surface-volume currently uses "
           "resolve strength.",
           ConfigOptionType::Float, bounded_range(0.0, 8.0), {}, {},
           ConfigOptionStability::Reference),
    option(RunConfigOptionId::CloudHorizonGlowStrength, "clouds.horizon_glow_strength",
           "--cloud-horizon-glow-strength", "Horizon Glow", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudSunGlareStrength, "clouds.sun_glare_strength",
           "--cloud-sun-glare-strength", "Sun Glare", kCloudLightingGroup,
    ...
    option(RunConfigOptionId::CloudJitterStrength, "clouds.jitter_strength",
           "--cloud-jitter-strength", "Jitter Strength", kCloudSamplingGroup,
    ...
    option(RunConfigOptionId::CloudOrbitTransitionStart, "clouds.orbit_transition_start_m",
           "--cloud-orbit-transition-start-m", "Orbit Transition Start", kCloudAerialOrbitGroup,
           "Deferred camera altitude where the broad orbit cloud shell starts blending in.",
           ConfigOptionType::Float, min_range(0.0), {}, {}, ConfigOptionStability::Deferred),
    option(RunConfigOptionId::CloudOrbitTransitionEnd, "clouds.orbit_transition_end_m",
           "--cloud-orbit-transition-end-m", "Orbit Transition End", kCloudAerialOrbitGroup,
           "Deferred camera altitude where the broad orbit cloud shell fully replaces local "
           "clouds.",
           ConfigOptionType::Float, min_range(0.0), {}, {}, ConfigOptionStability::Deferred),
    option(RunConfigOptionId::CloudFarShellStart, "clouds.far_shell_start_m",
           "--cloud-far-shell-start-m", "Far Shell Start", kCloudAerialOrbitGroup,
    ...
    option(RunConfigOptionId::CloudFarShellEnd, "clouds.far_shell_end_m", "--cloud-far-shell-end-m",
           "Far Shell End", kCloudAerialOrbitGroup,
    ...
    option(RunConfigOptionId::CloudFarShellStrength, "clouds.far_shell_strength",
           "--cloud-far-shell-strength", "Far Shell Strength", kCloudAerialOrbitGroup,
    ...
    option(RunConfigOptionId::CloudOrbitDetailStrength, "clouds.orbit_detail_strength",
           "--cloud-orbit-detail-strength", "Orbit Detail", kCloudAerialOrbitGroup,
    ...
    option(RunConfigOptionId::CloudOrbitDensityScale, "clouds.orbit_density_scale",
           "--cloud-orbit-density-scale", "Orbit Density", kCloudAerialOrbitGroup,
           "Deferred density multiplier for the broad orbit cloud shell.", ConfigOptionType::Float,
           bounded_range(0.0, 2.0), {}, {}, ConfigOptionStability::Deferred),
    option(RunConfigOptionId::CloudOrbitFill, "clouds.orbit_fill", "--cloud-orbit-fill",
           "Orbit Fill", kCloudAerialOrbitGroup,
           "Deferred fill bias for broad orbit cloud weather coverage.", ConfigOptionType::Float,
           bounded_range(0.0, 2.0), {}, {}, ConfigOptionStability::Deferred),
    option(RunConfigOptionId::CloudOrbitMotionStrength, "clouds.orbit_motion_strength",
           "--cloud-orbit-motion-strength", "Orbit Motion", kCloudAerialOrbitGroup,
    ...
    option(RunConfigOptionId::CloudOrbitShellExtinction, "clouds.orbit_shell_extinction",
           "--cloud-orbit-shell-extinction", "Orbit Extinction", kCloudAerialOrbitGroup,
    ...
    option(RunConfigOptionId::CloudTemporal, "clouds.temporal", "--cloud-temporal", "Temporal",
           kCloudSamplingGroup,
           "Enable experimental temporal reconstruction for the cloud product; off by default "
           "until shimmer is solved.",
    ...
    option(RunConfigOptionId::CloudLocalVolume, "clouds.local_volume", "--cloud-local-volume",
           "Local Volume", kCloudShapeGroup, "Enable near and overhead volumetric cloud marching.",
    ...
    option(RunConfigOptionId::CloudHorizonLayer, "clouds.horizon_layer", "--cloud-horizon-layer",
           "Horizon Layer", kCloudShapeGroup,
...
nlohmann::json option_to_json(const RunConfig& config, const ConfigOptionDescriptor& option) {
    ...
    case RunConfigOptionId::OceanMeshCells:
        return config.ocean.mesh_cells == 0U ? nlohmann::json(nullptr)
                                             : nlohmann::json(config.ocean.mesh_cells);
    case RunConfigOptionId::OceanMeshLodLevels:
        return config.ocean.mesh_lod_levels == 0U
                   ? nlohmann::json(nullptr)
    ...
    case RunConfigOptionId::OceanDetailAntiRepeatStrength:
        return optional_float(config.ocean.detail_anti_repeat_strength);
    ...
    case RunConfigOptionId::TerrainPlacementIndex:
        return config.terrain.placement_index;
    case RunConfigOptionId::TerrainForegroundHeight:
        return optional_float(config.terrain.foreground_height_m);
...
void set_run_config_option_from_string(RunConfig& config, const ConfigOptionDescriptor& option,
    ...
    case RunConfigOptionId::OceanCameraPreset:
        ...
        break;
    case RunConfigOptionId::OceanMeshCells:
        config.ocean.mesh_cells =
            parse_number<std::uint32_t>(value, option, "unsigned integer");
        validate_range(config.ocean.mesh_cells, option);
        break;
    case RunConfigOptionId::OceanMeshLodLevels:
        config.ocean.mesh_lod_levels =
            parse_number<std::uint32_t>(value, option, "unsigned integer");
        validate_range(config.ocean.mesh_lod_levels, option);
        break;
    case RunConfigOptionId::OceanHorizonTargetNearCell:
    ...
    case RunConfigOptionId::OceanShapeAntiRepeatStrength:
        ...
        validate_range(config.ocean.shape_anti_repeat_strength, option);
        break;
    case RunConfigOptionId::OceanDetailAntiRepeatStrength:
        config.ocean.detail_anti_repeat_strength = parse_config_float(value, option);
        validate_range(config.ocean.detail_anti_repeat_strength, option);
        break;
    case RunConfigOptionId::OceanDetailFilter:
    ...
    case RunConfigOptionId::TerrainPlacementIndex:
        ...
        break;
    case RunConfigOptionId::TerrainForegroundHeight:
        config.terrain.foreground_height_m = parse_config_float(value, option);
        validate_range(config.terrain.foreground_height_m, option);
        break;
    case RunConfigOptionId::TerrainCameraPreset:
...
void write_run_config_template(const RunConfig& config, const std::filesystem::path& path) {
    ...
    for (const ConfigOptionDescriptor& option : run_config_option_descriptors()) {
        nlohmann::json value = option_to_json(config, option);
        if (option.type == ConfigOptionType::Enum && value.is_string() &&
            value.get_ref<const std::string&>().empty()) {
            value = nullptr;
        }
        set_json_path(document, option.path, std::move(value));
    }

    bool output_path_set = false;
    validate_json_options(document, {}, output_path_set);
