...
void test_shader_bytecode_reads_aligned_spirv_words() {
    const std::filesystem::path path =
        std::filesystem::temp_directory_path() / "cubey_shader_bytecode_test.spv";
    ...
    const std::vector<std::uint32_t> loaded = cubey::vulkan::read_spirv_file(path);
...
void test_shader_bytecode_rejects_misaligned_spirv_byte_count() {
    const std::filesystem::path path =
        std::filesystem::temp_directory_path() / "cubey_shader_bytecode_bad_test.spv";
    std::filesystem::remove(path);
    ...
    bool rejected = false;
    try {
        static_cast<void>(cubey::vulkan::read_spirv_file(path));
