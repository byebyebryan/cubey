...
cubey::math::Mat4 HeadlessCubeApp::cube_mvp(VkExtent2D extent) const {
    ...
    const cubey::Transform3D cube_transform = cubey::examples::common::cube_spin_transform(0.75F);
    return camera.view_projection_matrix(camera_transform, aspect) * cube_transform.affine_matrix();
}

void HeadlessCubeApp::render_png(VkCommandBuffer command_buffer,
    ...
    forward_pass().record_to_prepared_target(
        recorder, target, [this, &target](const cubey::vulkan::CommandRecorder& pass_recorder) {
            ...
            pass_recorder.push_constants(forward_pass().pipeline().layout(),
                                         ...
                                         PushConstants{.mvp = cube_mvp(target.extent)});
