...
layout(set = 0, binding = 0) uniform SkyFrame {
...
vec3 star_sample_direction(vec3 ray_direction) {
    float c = celestial.celestial_options.x;
    float s = celestial.celestial_options.y;
    vec3 rotated = vec3(
        ray_direction.x * c - ray_direction.z * s,
        ray_direction.y,
        ray_direction.x * s + ray_direction.z * c);
...
vec3 night_sky_radiance(vec3 ray_direction, float above_horizon, float horizon_extinction,
    ...
    float day_washout = clamp(celestial.night_options.w, 0.0, 1.0);
    float moon_mask = moon_star_visibility(ray_direction);
    float moon_alignment =
        max(dot(ray_direction, normalize(celestial.moon_direction_radius.xyz)), 0.0);
    float moon_washout = 1.0 - smoothstep(celestial.moon_options.x, 1.0, moon_alignment) *
                                    clamp(celestial.moon_options.y, 0.0, 1.0);
