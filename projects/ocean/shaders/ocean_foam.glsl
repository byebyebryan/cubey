...
#define RETURN_DETAIL_TEXTURE(source_texture)                                                       \
    if (filter_mode == 1) {                                                                         \
...
vec4 sample_normal(uint cascade, vec2 uv, float pixels_per_meter) {
    ...
    if (cascade == 0u) {
        RETURN_DETAIL_TEXTURE(normal_cascade0_texture);
    }
    if (cascade == 1u) {
        RETURN_DETAIL_TEXTURE(normal_cascade1_texture);
    }
    if (cascade == 2u) {
        RETURN_DETAIL_TEXTURE(normal_cascade2_texture);
    }
    if (cascade == 3u) {
        RETURN_DETAIL_TEXTURE(normal_cascade3_texture);
    }
    RETURN_DETAIL_TEXTURE(normal_cascade4_texture);
}

vec4 sample_foam(uint cascade, vec2 uv, float pixels_per_meter) {
    int filter_mode = ocean_detail_filter_mode();
    if (cascade == 0u) {
        RETURN_DETAIL_TEXTURE(foam_cascade0_texture);
    }
    if (cascade == 1u) {
        RETURN_DETAIL_TEXTURE(foam_cascade1_texture);
    }
    if (cascade == 2u) {
        RETURN_DETAIL_TEXTURE(foam_cascade2_texture);
    }
    if (cascade == 3u) {
        RETURN_DETAIL_TEXTURE(foam_cascade3_texture);
    }
    RETURN_DETAIL_TEXTURE(foam_cascade4_texture);
}

#undef RETURN_DETAIL_TEXTURE
