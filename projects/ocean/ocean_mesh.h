...
[[nodiscard]] inline float ocean_mesh_near_cell_size(const OceanConfig& config) {
    return (ocean_mesh_near_half_extent(config) * 2.0F) / static_cast<float>(config.mesh_cells);
}
...
[[nodiscard]] inline std::uint32_t ocean_mesh_cells_for_span(float span,
...
ocean_mesh_patch_vertex_count(const OceanMeshPatch& patch) {
    return patch.cells_x * patch.cells_z * 6U;
}

[[nodiscard]] inline std::uint32_t
ocean_mesh_patch_triangle_count(const OceanMeshPatch& patch) {
...
[[nodiscard]] inline OceanMeshPatchList
ocean_mesh_clipmap_patches(const OceanConfig& config) {
...
[[nodiscard]] inline std::uint32_t
ocean_mesh_total_triangle_count(const OceanConfig& config) {
