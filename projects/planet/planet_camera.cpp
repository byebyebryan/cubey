...
[[nodiscard]] cubey::math::Vec3 clamped_position(const PlanetConfig& config,
                                                 cubey::math::Vec3 position_m) {
    const float distance = clamped_distance(config, glm::length(position_m));
    return normalize_or(position_m, cubey::math::Vec3{0.0F, 0.0F, 1.0F}) * distance;
}
...
float planet_camera_distance_m(const PlanetCameraState& state) {
    return glm::length(state.position_m);
}
...
PlanetCameraState planet_camera_home_state(const PlanetConfig& config, float base_yaw_radians,
    ...
    PlanetCameraState state{
        .position_m =
            initial_rotation * cubey::math::Vec3{0.0F, 0.0F, planet_camera_home_distance_m(config)},
    };
...
void planet_camera_set_distance(PlanetCameraState& state, const PlanetConfig& config,
    ...
    state.position_m = normalize_or(state.position_m, cubey::math::Vec3{0.0F, 0.0F, 1.0F}) *
                       clamped_distance(config, distance_m);
}
...
void planet_camera_orbit_drag(PlanetCameraState& state, const PlanetConfig& config,
    ...
    cubey::math::Vec3 position = yaw * state.position_m;
    ...
    state.position_m = clamped_position(config, position);
...
void planet_camera_surface_look_drag(PlanetCameraState& state, const PlanetConfig& config,
    ...
    const cubey::math::Vec3 up =
        normalize_or(state.position_m, cubey::math::Vec3{0.0F, 1.0F, 0.0F});
    const cubey::math::Quat yaw = cubey::math::angle_axis_quat(
...
bool planet_camera_surface_move(PlanetCameraState& state, const PlanetConfig& config, float forward,
    ...
    const cubey::math::Vec3 moved_position =
        normalize_or(state.position_m + glm::normalize(tangent_motion) * step_m, state.position_m) *
        distance;
    const cubey::math::Vec3 new_up = normalize_or(moved_position, old_up);
