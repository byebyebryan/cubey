...
struct TerrainGenerateConfig {
    ...
    cubey::projects::terrain_hydrology_lab::TerrainExportOptions export_options{};
};
...
[[nodiscard]] TerrainGenerateConfig parse_config(int argc, char** argv) {
    ...
    if (config.output_dir.empty()) {
        throw std::runtime_error("terrain_hydrology_generate requires --terrain-output-dir");
    }
    config.request.generator_revision =
        cubey::projects::terrain_hydrology_lab::terrain_generator_revision_for_recipe(config.request.recipe_id);
    return config;
}
...
int main(int argc, char** argv) {
    try {
        ...
        const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct product =
            cubey::projects::terrain_hydrology_lab::generate_terrain_patch(config.request);
        cubey::projects::terrain_hydrology_lab::write_terrain_field_exports(product, config.output_dir,
                                                              config.export_options);
        std::cout << "terrain_hydrology_generate: recipe=" << product.request.recipe_id
        ...
        std::cerr << "terrain_hydrology_generate: " << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
