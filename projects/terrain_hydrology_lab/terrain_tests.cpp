...
void test_default_patch_contract() {
    const cubey::projects::terrain_hydrology_lab::TerrainPatchRequest request =
        cubey::projects::terrain_hydrology_lab::default_terrain_patch_request();
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct product =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
    ...
    for (const std::string_view name : {
             cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldMountainSupport,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldHeightM,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldSlope,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldCurvature,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldLocalReliefM,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldRoutingSurfaceM,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldRoutingFillDeltaM,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldFlowDirectionX,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldFlowDirectionZ,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldContributingAreaM2,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldStreamOrder,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldDischargeProxy,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldSinkMask,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldFlowBoundaryMask,
         }) {
    ...
    require(
        product.fields.summarize_field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM).span >
            100.0F,
        "default terrain source should carry meaningful elevation relief");
    const cubey::procedural::ScalarField2D& source_height =
        product.fields.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM);
    const cubey::procedural::ScalarField2D& final_height =
        product.fields.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldHeightM);
    ...
    require(product.fields.summarize_field(cubey::projects::terrain_hydrology_lab::kTerrainFieldRoutingFillDeltaM)
                    .min >= 0.0F,
            "routing fill delta should be non-negative");
    require(
        product.fields.summarize_field(cubey::projects::terrain_hydrology_lab::kTerrainFieldStreamOrder).max >=
            2.0F,
        "default terrain should expose a drainage hierarchy");
    const cubey::procedural::ScalarFieldStats discharge =
        product.fields.summarize_field(cubey::projects::terrain_hydrology_lab::kTerrainFieldDischargeProxy);
...
void test_source_and_derivatives_are_halo_invariant() {
    ...
    const cubey::procedural::FieldSet2D small_source =
        cubey::projects::terrain_hydrology_lab::sample_upland_mountain_fields_v1(small_grid, 9012U);
    const cubey::procedural::FieldSet2D large_source =
        cubey::projects::terrain_hydrology_lab::sample_upland_mountain_fields_v1(large_grid, 9012U);
    const cubey::procedural::SlopeCurvature2D small_derivatives =
        cubey::procedural::compute_slope_curvature(
            small_source.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM));
    const cubey::procedural::SlopeCurvature2D large_derivatives =
        cubey::procedural::compute_slope_curvature(
            large_source.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM));
    ...
    for (std::uint32_t z = 0; z < interior_size; ++z) {
        for (std::uint32_t x = 0; x < interior_size; ++x) {
            ...
            require_near(small_source.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM)
                             .at(small_x, small_z),
                         large_source.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM)
                             .at(large_x, large_z),
...
void test_priority_flood_repairs_a_pit() {
    ...
    const cubey::projects::terrain_hydrology_lab::TerrainHydrologyResult result =
        cubey::projects::terrain_hydrology_lab::compute_regional_hydrology(height, 0U);
...
void test_landscape_graph_breaches_depressions_and_conserves_area() {
    ...
    const cubey::projects::terrain_hydrology_lab::TerrainLandscapeGraph graph =
        cubey::projects::terrain_hydrology_lab::build_terrain_landscape_graph(height, 9012U, 0U);
...
void test_landscape_graph_is_deterministic_and_seed_sensitive() {
    ...
    const auto first = cubey::projects::terrain_hydrology_lab::build_terrain_landscape_graph(height, 9012U, 0U);
    const auto repeat = cubey::projects::terrain_hydrology_lab::build_terrain_landscape_graph(height, 9012U, 0U);
    const auto changed =
        cubey::projects::terrain_hydrology_lab::build_terrain_landscape_graph(height, 12345U, 0U);
...
void test_landscape_graph_uses_all_lower_neighbors_for_gradient_correction() {
    ...
    const auto plane_graph =
        cubey::projects::terrain_hydrology_lab::build_terrain_landscape_graph(plane, 9012U, 0U);
    const auto diagonal_graph =
        cubey::projects::terrain_hydrology_lab::build_terrain_landscape_graph(diagonal, 9012U, 0U);
...
void test_landscape_multigrid_resampling_preserves_constant_fields() {
    ...
    const cubey::procedural::ScalarField2D downsampled =
        cubey::projects::terrain_hydrology_lab::downsample_terrain_landscape_field(field);
    ...
    const cubey::procedural::ScalarField2D upsampled =
        cubey::projects::terrain_hydrology_lab::upsample_terrain_landscape_field(downsampled, 9012U, 0U);
...
void test_landscape_upsampling_jitter_is_deterministic() {
    ...
    const auto first = cubey::projects::terrain_hydrology_lab::upsample_terrain_landscape_field(field, 9012U, 1U);
    const auto repeat =
        cubey::projects::terrain_hydrology_lab::upsample_terrain_landscape_field(field, 9012U, 1U);
    const auto changed =
        cubey::projects::terrain_hydrology_lab::upsample_terrain_landscape_field(field, 12345U, 1U);
...
void test_landscape_evolution_age_zero_preserves_initial_height() {
    ...
    cubey::projects::terrain_hydrology_lab::TerrainLandscapeEvolutionConfig config{};
    config.seed = 9012U;
    ...
    const auto result = cubey::projects::terrain_hydrology_lab::evolve_terrain_landscape(height, uplift, config);
...
void test_landscape_evolution_recipe_contract() {
    cubey::projects::terrain_hydrology_lab::TerrainPatchRequest request =
        cubey::projects::terrain_hydrology_lab::default_terrain_patch_request();
    request.recipe_id =
        std::string(cubey::projects::terrain_hydrology_lab::kTerrainRecipeUplandLandscapeEvolutionV1);
    request.generator_revision = cubey::projects::terrain_hydrology_lab::kTerrainUplandLandscapeEvolutionRevision;
    ...
    const auto first = cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
    const auto repeat = cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
    require(first.fields.field_count() == 29U,
            "landscape recipe should publish source, process, and diagnostic fields");
    for (const std::string_view name : {
             cubey::projects::terrain_hydrology_lab::kTerrainFieldUpliftRateMPerYear,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldProcessDrainageAreaM2,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldProcessFlowDirectionX,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldProcessFlowDirectionZ,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldProcessBreachMask,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldFluvialAdvectionRateMPerYear,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldHillslopeAdvectionRateMPerYear,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldThermalActiveMask,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldAnalyticalHeightM,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldAltitudeCorrectionDeltaM,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldProcessDeltaM,
         }) {
    ...
    require(
        !std::equal(
            first.fields.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM)
                .values()
                .begin(),
            first.fields.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM).values().end(),
            first.fields.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldHeightM).values().begin()),
    ...
    cubey::projects::terrain_hydrology_lab::write_terrain_manifest(first, output_dir);
    ...
    require(manifest.at("generation_scope") == "regional-not-seam-safe" &&
                manifest.at("process_halo_samples") ==
                    cubey::projects::terrain_hydrology_lab::kTerrainLandscapeProcessHaloSamples,
    ...
    const auto changed = cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
...
void test_patch_determinism_and_seed_variation() {
    ...
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct first =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct repeat =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
    require(first.summary.content_hash == repeat.summary.content_hash,
            "matching terrain requests should have matching hashes");

    request.domain.seed += 1U;
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct changed =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
    require(first.summary.content_hash != changed.summary.content_hash,
            "terrain seed should change the product hash");

    request.domain.seed = first.request.domain.seed + (1ULL << 32U);
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct upper_bits_changed =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
...
void test_broad_noise_control_contract() {
    ...
    for (const std::uint64_t seed : {0ULL, 9012ULL, 12345ULL}) {
        cubey::projects::terrain_hydrology_lab::TerrainPatchRequest request =
            cubey::projects::terrain_hydrology_lab::default_terrain_patch_request();
        request.recipe_id =
            std::string(cubey::projects::terrain_hydrology_lab::kTerrainRecipeUplandBroadNoiseControlV1);
        request.generator_revision =
            cubey::projects::terrain_hydrology_lab::kTerrainUplandBroadNoiseControlRevision;
        ...
        for (const std::string_view name : {
                 cubey::projects::terrain_hydrology_lab::kTerrainFieldUpliftPotential,
                 cubey::projects::terrain_hydrology_lab::kTerrainFieldMacroMass,
                 cubey::projects::terrain_hydrology_lab::kTerrainFieldBaseReliefM,
             }) {
        ...
        const cubey::procedural::ScalarFieldStats source =
            product.fields.summarize_field(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM);
        const cubey::procedural::ScalarFieldStats support =
            product.fields.summarize_field(cubey::projects::terrain_hydrology_lab::kTerrainFieldMountainSupport);
    ...
    cubey::projects::terrain_hydrology_lab::TerrainPatchRequest base =
        cubey::projects::terrain_hydrology_lab::default_terrain_patch_request();
    base.recipe_id = std::string(cubey::projects::terrain_hydrology_lab::kTerrainRecipeUplandBroadNoiseControlV1);
    base.generator_revision = cubey::projects::terrain_hydrology_lab::kTerrainUplandBroadNoiseControlRevision;
    base.domain.interior_grid.width = 33U;
    base.domain.interior_grid.height = 33U;
    base.domain.seed = 9012U;
    cubey::projects::terrain_hydrology_lab::TerrainPatchRequest upper_seed = base;
    upper_seed.domain.seed += 1ULL << 32U;
    require(cubey::projects::terrain_hydrology_lab::generate_terrain_patch(base).summary.content_hash !=
                cubey::projects::terrain_hydrology_lab::generate_terrain_patch(upper_seed).summary.content_hash,
            "broad-noise control should consume terrain seed upper bits");

    cubey::projects::terrain_hydrology_lab::TerrainPatchRequest right = base;
    ...
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct left_product =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(base);
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct right_product =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(right);
    for (const std::string_view name : {
             cubey::projects::terrain_hydrology_lab::kTerrainFieldUpliftPotential,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldMacroMass,
             cubey::projects::terrain_hydrology_lab::kTerrainFieldBaseReliefM,
...
void test_adjacent_patch_source_seam() {
    cubey::projects::terrain_hydrology_lab::TerrainPatchRequest left =
        cubey::projects::terrain_hydrology_lab::default_terrain_patch_request();
    left.domain.interior_grid.width = 33U;
    left.domain.interior_grid.height = 33U;
    cubey::projects::terrain_hydrology_lab::TerrainPatchRequest right = left;
    ...
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct left_product =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(left);
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct right_product =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(right);
    for (const std::string_view name : {
             cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM,
...
void test_request_validation() {
    ...
    require_throws(
        [&request] { cubey::projects::terrain_hydrology_lab::validate_terrain_patch_request(request); },
        "terrain request should reject even dimensions");

    request = cubey::projects::terrain_hydrology_lab::default_terrain_patch_request();
...
void test_scalar_export_and_manifest() {
    ...
    std::filesystem::remove_all(output_dir);
    cubey::projects::terrain_hydrology_lab::write_terrain_field_exports(
        product, output_dir,
        cubey::projects::terrain_hydrology_lab::TerrainExportOptions{.write_raw_float32 = true});
    ...
    require(manifest.at("process_halo_samples") ==
                cubey::projects::terrain_hydrology_lab::kTerrainProcessHaloSamples,
    ...
    const nlohmann::json& source_entry =
        manifest.at("fields").at(cubey::projects::terrain_hydrology_lab::kTerrainFieldSourceHeightM);
...
void test_raw_scalar_export_is_little_endian_row_major() {
    ...
    const std::vector<std::uint8_t> bytes =
        cubey::projects::terrain_hydrology_lab::encode_terrain_scalar_field_f32_le(field);
...
void test_fixed_field_display_ranges() {
    ...
    const cubey::projects::terrain_hydrology_lab::TerrainFieldDisplaySpec first_display =
        cubey::projects::terrain_hydrology_lab::terrain_field_display_spec(
            cubey::projects::terrain_hydrology_lab::kTerrainFieldSlope, first);
    const cubey::projects::terrain_hydrology_lab::TerrainFieldDisplaySpec second_display =
        cubey::projects::terrain_hydrology_lab::terrain_field_display_spec(
            cubey::projects::terrain_hydrology_lab::kTerrainFieldSlope, second);
    ...
    require_near(cubey::projects::terrain_hydrology_lab::terrain_field_display_value(1.0F, first_display),
                 cubey::projects::terrain_hydrology_lab::terrain_field_display_value(1.0F, second_display), 0.0F,
...
void test_terrain_mesh_consumes_product_fields() {
    ...
    const cubey::projects::terrain_hydrology_lab::TerrainPatchProduct product =
        cubey::projects::terrain_hydrology_lab::generate_terrain_patch(request);
    const cubey::projects::terrain_hydrology_lab::TerrainMeshData surface =
        cubey::projects::terrain_hydrology_lab::make_terrain_mesh(product, "surface", 1.0F);
    const cubey::projects::terrain_hydrology_lab::TerrainMeshData flow =
        cubey::projects::terrain_hydrology_lab::make_terrain_mesh(product, "flow-direction", 1.0F);
    ...
    require(
        surface.vertices.front().position[1] ==
            product.fields.field(cubey::projects::terrain_hydrology_lab::kTerrainFieldHeightM).values().front(),
    ...
    require_throws(
        [&product] {
            static_cast<void>(
                cubey::projects::terrain_hydrology_lab::make_terrain_mesh(product, "not-a-field", 1.0F));
        },
...
int main() {
    try {
        ...
        std::cout << "terrain_hydrology_lab_tests: ok\n";
        return EXIT_SUCCESS;
    } catch (const std::exception& error) {
        std::cerr << "terrain_hydrology_lab_tests: " << error.what() << '\n';
        return EXIT_FAILURE;
    }
}
