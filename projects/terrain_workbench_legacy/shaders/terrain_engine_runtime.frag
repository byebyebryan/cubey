...
void main() {
    ...
    float lighting = 0.34 + diffuse * 0.62 + sky * 0.18 + rim * 0.08;
    vec3 color = frag_color * lighting;

    float extent = max(pc.light_direction_extent.w, 1.0);
    float fog = smoothstep(extent * 0.45, extent * 1.10, length(frag_world_position.xz));
    color = mix(color, vec3(0.57, 0.65, 0.68), fog * 0.16);
