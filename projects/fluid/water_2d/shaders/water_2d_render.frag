...
void main() {
    ...
    float density = particle_density(uv, width, height, radius, aspect, max_particles_per_cell);
    float water = smoothstep(0.05, 0.82, density);
    float surface = smoothstep(0.12, 0.56, density) * (1.0 - smoothstep(0.62, 1.40, density));
    ...
    color += vec3(0.50, 0.80, 0.96) * surface * 0.52;
