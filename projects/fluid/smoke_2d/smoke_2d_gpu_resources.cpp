...
void Smoke2DGpuResources::create_descriptor_resources(cubey::vulkan::Device& device) {
    ...
    const std::array<cubey::vulkan::DescriptorSetBindingConfig, 2> pressure_rbgs_bindings{{
        {
            .binding = 0,
            .type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
            .stage_flags = VK_SHADER_STAGE_COMPUTE_BIT,
    ...
    const std::array<cubey::vulkan::DescriptorSetBindingConfig, 5> render_bindings{{
        {
            .binding = 0,
            .type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
...
void Smoke2DGpuResources::update_field_descriptors(cubey::vulkan::Device& device) {
    ...
    descriptor_writes
        ...
        .storage_buffer(advect_descriptor_set_, 2, injectors().handle(), injectors().size())
        ...
        .storage_buffer(inject_descriptor_set_, 2, injectors().handle(), injectors().size())
        ...
        .storage_buffer(curl_descriptor_set_, 2, injectors().handle(), injectors().size())
        ...
        .storage_buffer(vorticity_descriptor_set_, 2, injectors().handle(), injectors().size());
