...
[[nodiscard]] Rgb surface_color(const TerrainPatchProduct& product, std::uint32_t x,
    ...
    const float h = terrain_field_display_value(
        height.at(x, z), terrain_field_display_spec(kTerrainFieldHeightM, height));
...
[[nodiscard]] Rgb scalar_color(const TerrainPatchProduct& product, const ViewSelection& selection,
    ...
    const TerrainFieldDisplaySpec display =
        terrain_field_display_spec(selection.field_name, field);
    ...
    if (selection.field_name == kTerrainFieldStreamOrder) {
        return mix({0.08F, 0.10F, 0.12F}, {0.90F, 0.74F, 0.18F}, t);
    }
    ...
    if (display.palette == TerrainFieldDisplayPalette::Diverging) {
        const float signed_value = (t * 2.0F) - 1.0F;
        return signed_value < 0.0F
                   ? mix({0.84F, 0.86F, 0.88F}, {0.08F, 0.32F, 0.84F}, -signed_value)
                   : mix({0.84F, 0.86F, 0.88F}, {0.90F, 0.18F, 0.08F}, signed_value);
    }
    ...
    for (std::uint32_t z = 0; z < desc.height; ++z) {
        for (std::uint32_t x = 0; x < desc.width; ++x) {
            const Rgb color = selection.kind == ViewKind::Surface
                                  ? surface_color(product, x, z)
                                  : scalar_color(product, selection, x, z);
