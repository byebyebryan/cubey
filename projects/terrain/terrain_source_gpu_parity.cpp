...
static_assert(sizeof(TerrainSourceParityInput) == 240U);
static_assert(sizeof(TerrainSourceParityOutput) == 192U);
    ...
    for (const auto preset : presets) {
        for (const auto weathering : weathering_modes) {
            ...
            for (std::size_t index = 0; index < queries.size(); ++index) {
                input_data.queries[index] = {queries[index].world_xz.x, queries[index].world_xz.y,
