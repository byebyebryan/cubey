...
void main() {
    ...
    vec2 level_origin = floor(camera_xz / cell_size_m) * cell_size_m;
    ...
    float height_x = terrain_source_base_height(
        terrain_uniforms.source, world_xz + vec2(normal_step_m, 0.0), footprint_m);
    float height_z = terrain_source_base_height(
        terrain_uniforms.source, world_xz + vec2(0.0, normal_step_m), footprint_m);
