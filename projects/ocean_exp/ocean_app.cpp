...
#ifndef CUBEY_OCEAN_EXP_SHADER_DIR
#error "CUBEY_OCEAN_EXP_SHADER_DIR must be defined by the ocean_exp CMake target"
#endif
  ...
  private:
        ...
        return cubey::host::run_windowed_app(
            {
                .run_config = config_,
                .app_name = "ocean_exp",
                .ready_status = "rendering ocean experimental snapshot",
    ...
    void create_pipeline(cubey::vulkan::Device& device, cubey::vulkan::GpuRuntime& gpu,
        ...
        ocean_gpu_.create(device, OceanGpuResourceConfig{
                                      .ocean = ocean_config_,
                                      .shader_dir = CUBEY_OCEAN_EXP_SHADER_DIR,
        ...
        const std::array<cubey::render::ShaderStageFile, 2> post_shader_stage_files{
            cubey::render::ShaderStageFile{
                ...
                .path = std::filesystem::path(CUBEY_OCEAN_EXP_SHADER_DIR) / "forward_pbr_post.vert.spv",
            },
            cubey::render::ShaderStageFile{
                .stage = VK_SHADER_STAGE_FRAGMENT_BIT,
                .path = std::filesystem::path(CUBEY_OCEAN_EXP_SHADER_DIR) / "forward_pbr_post.frag.spv",
            },
        };
    ...
    void create_atmosphere_environment_runtime(cubey::vulkan::Device& device,
        ...
        if (!atmosphere_runtime_.resources_created()) {
            ...
            const std::filesystem::path shader_dir = CUBEY_OCEAN_EXP_SHADER_DIR;
...
int run_ocean_exp(const RunConfig& config) {
    OceanApp app(config);
    return app.run();
}
