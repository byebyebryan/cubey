...
struct Fluid3DConfig {
    ...
    float fire_burn_rate = 3.4F;
    float fire_heat_output = 2.6F;
    float fire_soot_yield = 0.16F;
    float fire_expansion = 1.35F;
    float fire_flame_cooling = 3.0F;
    float fire_shredding = 2.4F;
    float fire_turbulence = 0.55F;
...
[[nodiscard]] inline Fluid3DConfig fluid_3d_config_from_run_config(const RunConfig& config) {
    ...
    result.fire_ignition_temperature = config.fluid_fire_ignition_temperature;
    result.fire_burn_rate = config.fluid_fire_burn_rate;
    result.fire_heat_output = config.fluid_fire_heat_output;
    result.fire_soot_yield = config.fluid_fire_soot_yield;
    result.fire_expansion = config.fluid_fire_expansion;
    result.fire_flame_cooling = config.fluid_fire_flame_cooling;
    result.fire_shredding = config.fluid_fire_shredding;
    result.fire_turbulence = config.fluid_fire_turbulence;
