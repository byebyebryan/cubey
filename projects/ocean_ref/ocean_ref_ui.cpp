...
void draw_map_size_combo(OceanRefConfig& config) {
    ...
    if (ImGui::BeginCombo("Map size", preview)) {
        for (const std::uint32_t map_size : kOceanRefSupportedMapSizes) {
...
void draw_cascade_controls(OceanRefCascadeConfig& cascade, std::uint32_t index) {
    char label[32]{};
...
void draw_ocean_ref_ui(OceanRefUiContext ui) {
    if (!cubey::host::begin_control_panel("Ocean Ref", {.width = 390.0F})) {
        ImGui::End();
        return;
    }
    ...
    if (cubey::host::imgui_section("Mesh", true)) {
        ...
        if (ImGui::SliderInt("Base cells", &mesh_cells, static_cast<int>(kOceanRefMinMeshCells),
                             static_cast<int>(kOceanRefMaxMeshCells))) {
            ui.config.mesh_cells = static_cast<std::uint32_t>(
                std::clamp(mesh_cells, static_cast<int>(kOceanRefMinMeshCells),
                           static_cast<int>(kOceanRefMaxMeshCells)));
        }
        int mesh_lod_levels = static_cast<int>(ui.config.mesh_lod_levels);
        if (ImGui::SliderInt("LOD levels", &mesh_lod_levels,
                             static_cast<int>(kOceanRefMinMeshLodLevels),
                             static_cast<int>(kOceanRefMaxMeshLodLevels))) {
            ui.config.mesh_lod_levels = static_cast<std::uint32_t>(
                std::clamp(mesh_lod_levels, static_cast<int>(kOceanRefMinMeshLodLevels),
                           static_cast<int>(kOceanRefMaxMeshLodLevels)));
        }
    ...
    if (cubey::host::imgui_section("Cascades", true)) {
        ...
        for (std::uint32_t index = 0; index < kOceanRefCascadeCount; ++index) {
