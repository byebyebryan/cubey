...
#ifndef CUBEY_OCEAN_REF_SHADER_DIR
#error "CUBEY_OCEAN_REF_SHADER_DIR must be defined by the ocean_ref CMake target"
#endif
...
constexpr VkFormat kOceanRefDepthFormat = VK_FORMAT_D32_SFLOAT;
constexpr float kGravity = 9.81F;

struct OceanRefPushConstants {
...
struct OceanRefSkyPushConstants {
    cubey::math::Vec4 camera_time;
...
struct OceanRefSpectrumPushConstants {
...
struct OceanRefModulatePushConstants {
    cubey::math::Vec4 tile_depth_time;
    cubey::math::Vec4 cascade_options;
};

struct OceanRefFftPushConstants {
    cubey::math::Vec4 fft_options;
    cubey::math::Vec4 pass_options;
};

struct OceanRefUnpackPushConstants {
    cubey::math::Vec4 foam_options;
    cubey::math::Vec4 cascade_options;
};

static_assert(sizeof(OceanRefPushConstants) == sizeof(float) * 56U);
static_assert(sizeof(OceanRefSkyPushConstants) == sizeof(float) * 20U);
static_assert(sizeof(OceanRefSpectrumPushConstants) == sizeof(float) * 16U);
static_assert(sizeof(OceanRefModulatePushConstants) == sizeof(float) * 8U);
static_assert(sizeof(OceanRefFftPushConstants) == sizeof(float) * 8U);
static_assert(sizeof(OceanRefUnpackPushConstants) == sizeof(float) * 8U);

enum class OceanRefRenderTargetMode : std::uint8_t {
    Present,
    ColorAttachment,
};

struct OceanRefFrameGraph {
...
[[nodiscard]] std::uint32_t log2_exact(std::uint32_t value) {
    if (!ocean_ref_is_power_of_two(value)) {
        throw std::runtime_error("ocean_ref FFT resolution must be a power of two");
    }
...
[[nodiscard]] std::uint32_t triangle_count(const OceanRefConfig& config) {
    return ocean_ref_mesh_total_triangle_count(config);
}

[[nodiscard]] cubey::render::ComputeDispatchGroups
ocean_ref_dispatch_groups(const OceanRefConfig& config) {
...
ocean_ref_depth_texture_desc(const char* label, VkExtent2D extent, VkFormat format) {
    return {
        .label = label,
...
[[nodiscard]] bool ocean_ref_resolution_changed(const OceanRefConfig& lhs,
                                                const OceanRefConfig& rhs) {
...
class OceanRefApp {
  public:
    ...
    OceanRefApp(const OceanRefApp&) = delete;
    OceanRefApp& operator=(const OceanRefApp&) = delete;
  ...
  private:
        ...
        return cubey::host::run_windowed_app(
            {
                .run_config = config_,
                .app_name = "ocean_ref",
                .ready_status = "rendering ocean reference project",
    ...
    int run_headless() {
        ...
        callbacks.record_frame = [this](cubey::host::HeadlessPngContext& context,
            ...
            record_ocean_ref_target(command_buffer, context.device(), target, frame.frame_slot,
                                    OceanRefRenderTargetMode::ColorAttachment);
        };
    ...
    void update_windowed(cubey::host::WindowedAppContext& context, const FrameTiming& timing) {
        ...
        if (input.key_pressed(cubey::input::Key::D)) {
            render_view_ = next_ocean_ref_render_view(render_view_);
        }
    ...
    void create_pipeline(cubey::vulkan::Device& device, VkFormat color_format, VkExtent2D extent,
        ...
        ocean_gpu_.create(device, OceanRefGpuResourceConfig{
                                      .ocean = ocean_config_,
                                      .shader_dir = CUBEY_OCEAN_REF_SHADER_DIR,
                                      .color_format = color_format,
                                      .depth_format = kOceanRefDepthFormat,
    ...
    void sync_gpu_resources(cubey::host::WindowedAppContext& context) {
        validate_ocean_ref_config(ocean_config_);
        if (!gpu_config_.has_value()) {
            create_pipeline(context.device(), context.swapchain().format(),
                            context.swapchain().extent(), context.frame_slot_count());
            return;
        }
        if (ocean_ref_resolution_changed(ocean_config_, gpu_config_.value())) {
            cubey::vulkan::check(vkDeviceWaitIdle(context.device().handle()),
                                 "vkDeviceWaitIdle before ocean_ref resource recreation");
            create_pipeline(context.device(), context.swapchain().format(),
                            context.swapchain().extent(), context.frame_slot_count());
            return;
        }
    ...
    [[nodiscard]] OceanRefPushConstants
    surface_push_constants(VkExtent2D extent, const OceanRefMeshPatch& patch) const {
    ...
    [[nodiscard]] OceanRefSkyPushConstants sky_push_constants(VkExtent2D extent) const {
        const cubey::Transform3D transform = camera_transform();
        const float aspect = static_cast<float>(extent.width) / static_cast<float>(extent.height);
    ...
    [[nodiscard]] OceanRefSpectrumPushConstants
    spectrum_push_constants(std::uint32_t cascade_index) const {
        const OceanRefCascadeConfig& cascade = ocean_ref_cascade(ocean_config_, cascade_index);
    ...
    [[nodiscard]] OceanRefModulatePushConstants
    ...
    [[nodiscard]] OceanRefFftPushConstants fft_push_constants(std::uint32_t stage, bool horizontal,
    ...
    [[nodiscard]] OceanRefUnpackPushConstants
    ...
    void record_ocean_ref_sky(const cubey::vulkan::CommandRecorder& recorder,
                              VkExtent2D extent) const {
        const OceanRefSkyPushConstants constants = sky_push_constants(extent);
    ...
    void record_ocean_ref_draw(const cubey::vulkan::CommandRecorder& recorder,
                               VkExtent2D extent) const {
        const OceanRefMeshPatchList patches = ocean_ref_mesh_clipmap_patches(ocean_config_);
        ...
        for (const OceanRefMeshPatch& patch : patches) {
            const OceanRefPushConstants constants = surface_push_constants(extent, patch);
            recorder.push_constants(surface_pipeline.layout(),
                                    VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                                    constants);
            recorder.draw(ocean_ref_mesh_patch_vertex_count(patch));
        }
    }

    [[nodiscard]] OceanRefFrameGraph
    build_ocean_ref_frame_graph(cubey::render::ColorTargetView color_target,
                                OceanRefRenderTargetMode target_mode) const {
        cubey::render::RenderGraphBuilder graph;
        const cubey::render::RenderGraphTextureState initial_state =
            target_mode == OceanRefRenderTargetMode::Present
        ...
        const cubey::render::RenderGraphTextureHandle backbuffer = graph.import_color_target(
            "ocean_ref backbuffer", color_target, initial_state, final_state);
        const cubey::render::RenderGraphTextureHandle surface_depth =
            graph.create_texture(ocean_ref_depth_texture_desc(
                "ocean_ref surface depth", color_target.extent, kOceanRefDepthFormat));

        graph.add_pass("ocean_ref final", cubey::render::RenderGraphQueueDomain::Graphics)
            ...
            .execute([this, backbuffer,
                ...
                cubey::render::record_render_target_pass(
                    ...
                    [this, target](const cubey::vulkan::CommandRecorder& draw_recorder) {
                        record_ocean_ref_sky(draw_recorder, target.extent);
                        record_ocean_ref_draw(draw_recorder, target.extent);
                    });
            });
    ...
    void record_spectrum_init(const cubey::vulkan::CommandRecorder& recorder) const {
        const cubey::render::ComputeDispatchGroups groups =
            ocean_ref_dispatch_groups(ocean_config_);
        for (std::uint32_t cascade = 0; cascade < kOceanRefCascadeCount; ++cascade) {
    ...
    void record_ocean_ref_compute(const cubey::vulkan::CommandRecorder& recorder) {
    ...
    void record_ocean_ref_target(VkCommandBuffer command_buffer,
                                 const cubey::vulkan::Device& device,
                                 cubey::render::ColorTargetView target,
                                 cubey::render::FrameSlot frame_slot,
                                 OceanRefRenderTargetMode target_mode) {
        const cubey::vulkan::CommandRecorder recorder(command_buffer);
        record_ocean_ref_compute(recorder);
        const OceanRefFrameGraph frame_graph = build_ocean_ref_frame_graph(target, target_mode);
        graph_executor_.record(
            cubey::render::RenderGraphFrameRecordInfo{
                .device = &device,
                .command_buffer = command_buffer,
                .frame_slot = frame_slot,
                .label = "vkEndCommandBuffer ocean_ref graph",
                .command_buffer_mode =
    ...
    void record_windowed_frame(const cubey::vulkan::Device& device,
        ...
        record_ocean_ref_target(frame.command_buffer, device, frame.color_target, frame.frame_slot,
                                OceanRefRenderTargetMode::Present);
        recorder.end("vkEndCommandBuffer ocean_ref");
    }
    ...
    OceanRefGpuResources ocean_gpu_;
    cubey::render::RenderGraphFrameExecutor graph_executor_;
    std::optional<OceanRefConfig> gpu_config_;
...
int run_ocean_ref(const RunConfig& config) {
    OceanRefApp app(config);
    return app.run();
}
