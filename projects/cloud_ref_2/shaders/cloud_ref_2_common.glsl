#ifndef CUBEY_CLOUD_REF_2_COMMON_GLSL
#define CUBEY_CLOUD_REF_2_COMMON_GLSL

const float CLOUD_REF_2_PI = 3.14159265359;

const int CLOUD_REF_2_DEBUG_FINAL = 0;
const int CLOUD_REF_2_DEBUG_WEATHER = 1;
const int CLOUD_REF_2_DEBUG_DENSITY = 2;
const int CLOUD_REF_2_DEBUG_TRANSMITTANCE = 3;
const int CLOUD_REF_2_DEBUG_LIGHTING = 4;
const int CLOUD_REF_2_DEBUG_SHADOW = 5;
const int CLOUD_REF_2_DEBUG_STEPS = 6;
const int CLOUD_REF_2_DEBUG_BACKGROUND = 7;
const int CLOUD_REF_2_DEBUG_RAW_FINAL = 8;
const int CLOUD_REF_2_DEBUG_CLOUD_ALPHA = 11;
const int CLOUD_REF_2_DEBUG_DISTANCE = 15;
const int CLOUD_REF_2_DEBUG_BASE_DENSITY = 16;
const int CLOUD_REF_2_DEBUG_DETAIL_DENSITY = 17;
...
float cloud_ref_2_remap(float value, float old_min, float old_max, float new_min, float new_max) {
    return new_min + ((value - old_min) / max(old_max - old_min, 0.00001)) *
                         (new_max - new_min);
}

vec3 cloud_ref_2_view_direction(vec2 position) {
...
float cloud_ref_2_planet_radius() {
    return params.camera_position_radius.w;
}

float cloud_ref_2_camera_altitude() {
    return params.camera_position_radius.y;
}

vec3 cloud_ref_2_sphere_center() {
    return vec3(params.camera_position_radius.x, -cloud_ref_2_planet_radius(),
                params.camera_position_radius.z);
}

float cloud_ref_2_inner_radius() {
    return cloud_ref_2_planet_radius() + params.cloud_shell.x;
}

float cloud_ref_2_outer_radius() {
    return cloud_ref_2_inner_radius() + params.cloud_shell.y;
}

bool cloud_ref_2_ray_sphere(vec3 origin, vec3 direction, float radius, out float near_t,
                          out float far_t) {
    vec3 local_origin = origin - cloud_ref_2_sphere_center();
...
float cloud_ref_2_hg(float cos_theta, float g) {
    float g2 = g * g;
    float denom = pow(max(1.0 + g2 - 2.0 * g * cos_theta, 0.001), 1.5);
    return (1.0 - g2) / (4.0 * CLOUD_REF_2_PI * denom);
}

float cloud_ref_2_phase(float cos_theta) {
    float blend = cloud_ref_2_saturate(cos_theta * 0.5 + 0.5);
    float phase = mix(cloud_ref_2_hg(cos_theta, -0.08), cloud_ref_2_hg(cos_theta, 0.08), blend);
    return max(phase, 1.0);
}

float cloud_ref_2_hash(vec2 p) {
    vec3 p3 = fract(vec3(p.xyx) * 0.1031);
    p3 += dot(p3, p3.yzx + 33.33);
    return fract((p3.x + p3.y) * p3.z);
}

float cloud_ref_2_value_noise(vec2 p) {
    vec2 i = floor(p);
    vec2 f = fract(p);
    vec2 u = f * f * (3.0 - 2.0 * f);
    return mix(mix(cloud_ref_2_hash(i + vec2(0.0, 0.0)), cloud_ref_2_hash(i + vec2(1.0, 0.0)), u.x),
               mix(cloud_ref_2_hash(i + vec2(0.0, 1.0)), cloud_ref_2_hash(i + vec2(1.0, 1.0)), u.x),
               u.y);
}

float cloud_ref_2_starfield(vec3 direction) {
    vec2 p = vec2(atan(direction.z, direction.x) / (2.0 * CLOUD_REF_2_PI) + 0.5,
                  asin(clamp(direction.y, -1.0, 1.0)) / CLOUD_REF_2_PI + 0.5);
    float stars = cloud_ref_2_value_noise(p * 900.0);
    return smoothstep(0.993, 1.0, stars);
}

vec3 cloud_ref_2_sky_color(vec3 direction) {
    vec3 up = cloud_ref_2_planet_up();
    ...
    sky += vec3(cloud_ref_2_starfield(direction)) * (1.0 - day) * 0.9;
    return sky;
}

vec3 cloud_ref_2_water_context(vec3 direction) {
    vec3 up = cloud_ref_2_planet_up();
    vec3 sun_dir = normalize(params.sun_direction_intensity.xyz);
    float view_height = clamp(dot(direction, up), -1.0, 1.0);
    float camera_height = max(cloud_ref_2_camera_altitude(), 1.0);
    ...
    float broad = cloud_ref_2_value_noise(ripple_uv * 2.1);
    float detail = cloud_ref_2_value_noise(ripple_uv * 11.0 + vec2(13.7, 5.1));
    float wave = mix(broad, detail, 0.32);

    vec3 reflection_dir = reflect(direction, up);
    vec3 reflected_sky = cloud_ref_2_sky_color(reflection_dir);
...
vec3 cloud_ref_2_background(vec3 direction) {
    vec3 up = cloud_ref_2_planet_up();
    float view_height = clamp(dot(direction, up), -1.0, 1.0);
    vec3 sky = cloud_ref_2_sky_color(direction);

    if (view_height < -0.015) {
        vec3 water = cloud_ref_2_water_context(direction);
        sky = mix(water, sky, smoothstep(-0.045, 0.018, view_height));
    }
    return sky;
}

vec3 cloud_ref_2_tonemap(vec3 color) {
