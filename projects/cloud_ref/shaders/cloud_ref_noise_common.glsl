...
const float kCloudRefFrequencyMul[6] = float[6](2.0, 8.0, 14.0, 20.0, 26.0, 32.0);

float cloud_ref_hash_int(int n) {
    return fract(sin(float(n) + 1.951) * 43758.5453123);
}

float cloud_ref_value_noise(vec3 x) {
    vec3 p = floor(x);
    vec3 f = fract(x);
    ...
    return mix(
        mix(mix(cloud_ref_hash_int(int(n + 0.0)), cloud_ref_hash_int(int(n + 1.0)), f.x),
            mix(cloud_ref_hash_int(int(n + 57.0)), cloud_ref_hash_int(int(n + 58.0)), f.x),
            f.y),
        mix(mix(cloud_ref_hash_int(int(n + 113.0)), cloud_ref_hash_int(int(n + 114.0)), f.x),
            mix(cloud_ref_hash_int(int(n + 170.0)), cloud_ref_hash_int(int(n + 171.0)), f.x),
            f.y),
        f.z);
}

float cloud_ref_cells(vec3 p, float cell_count) {
    ...
    for (int xo = -1; xo <= 1; ++xo) {
        for (int yo = -1; yo <= 1; ++yo) {
            for (int zo = -1; zo <= 1; ++zo) {
                ...
                tp = p_cell - tp - cloud_ref_value_noise(mod(tp, cell_count));
                d = min(d, dot(tp, tp));
            }
        }
    }
...
vec4 cloud_ref_mod289(vec4 x) {
    return x - floor(x * vec4(1.0 / 289.0)) * vec4(289.0);
}

vec4 cloud_ref_permute(vec4 x) {
    return cloud_ref_mod289(((x * 34.0) + 1.0) * x);
}

vec4 cloud_ref_taylor_inv_sqrt(vec4 r) {
    return vec4(1.79284291400159) - vec4(0.85373472095314) * r;
}

vec4 cloud_ref_fade(vec4 t) {
    return (t * t * t) * (t * (t * vec4(6.0) - vec4(15.0)) + vec4(10.0));
}

float cloud_ref_glm_perlin_4d(vec4 position, vec4 rep) {
    ...
    vec4 ixy = cloud_ref_permute(cloud_ref_permute(ix) + iy);
    vec4 ixy0 = cloud_ref_permute(ixy + iz0);
    vec4 ixy1 = cloud_ref_permute(ixy + iz1);
    vec4 ixy00 = cloud_ref_permute(ixy0 + iw0);
    vec4 ixy01 = cloud_ref_permute(ixy0 + iw1);
    vec4 ixy10 = cloud_ref_permute(ixy1 + iw0);
    vec4 ixy11 = cloud_ref_permute(ixy1 + iw1);
    ...
    vec4 norm00 = cloud_ref_taylor_inv_sqrt(
    ...
    vec4 norm01 = cloud_ref_taylor_inv_sqrt(
    ...
    vec4 norm10 = cloud_ref_taylor_inv_sqrt(
    ...
    vec4 norm11 = cloud_ref_taylor_inv_sqrt(
    ...
    vec4 fade_xyzw = cloud_ref_fade(pf0);
...
float cloud_ref_worley_noise_3d(vec3 p, float cell_count) {
    return cloud_ref_cells(p, cell_count);
}

float cloud_ref_perlin_noise_3d(vec3 p_in, float frequency, int octave_count) {
    float sum = 0.0;
    float weight_sum = 0.0;
    float weight = 0.5;
    for (int oct = 0; oct < octave_count; ++oct) {
        vec4 p = vec4(p_in.x, p_in.y, p_in.z, 0.0) * vec4(frequency);
        float value = cloud_ref_glm_perlin_4d(p, vec4(frequency));
        sum += value * weight;
        weight_sum += weight;
        weight *= weight;
        frequency *= 2.0;
...
const vec3 kCloudRefWeatherSeed = vec3(0.0);

float cloud_ref_random_2d(vec2 st) {
    return fract(sin(dot(st.xy, vec2(12.9898, 78.233) + kCloudRefWeatherSeed.xy)) *
                 43758.5453123);
}

float cloud_ref_noise_interpolation_2d(vec2 coord, float size) {
    vec2 grid = coord * size;
    vec2 random_input = floor(grid);
    vec2 weights = fract(grid);

    float p0 = cloud_ref_random_2d(random_input);
    float p1 = cloud_ref_random_2d(random_input + vec2(1.0, 0.0));
    float p2 = cloud_ref_random_2d(random_input + vec2(0.0, 1.0));
    float p3 = cloud_ref_random_2d(random_input + vec2(1.0, 1.0));
...
float cloud_ref_perlin_noise_2d(vec2 uv, float scale, float frequency, float amplitude,
                                int octaves) {
    float noise_value = 0.0;
    float local_amplitude = amplitude;
    float local_frequency = frequency;
    for (int index = 0; index < octaves; ++index) {
        noise_value += cloud_ref_noise_interpolation_2d(uv, scale * local_frequency) *
                       local_amplitude;
