...
[[nodiscard]] inline float ocean_horizon_effective_near_cell_target_m(
    const OceanSurfaceConfig& config, float camera_altitude_m) {
...
[[nodiscard]] inline std::uint32_t ocean_horizon_effective_mesh_cells(
    ...
    OceanSurfaceConfig mesh_config = config;
...
ocean_horizon_diagnostics(const OceanSurfaceConfig& config,
                          float camera_y_m,
...
[[nodiscard]] inline float ocean_horizon_near_cell_size_for_lod(const OceanSurfaceConfig& config,
                                                                float mesh_half_extent_m,
                                                                std::uint32_t lod_levels) {
    OceanSurfaceConfig mesh_config = config;
    mesh_config.mesh_extent = mesh_half_extent_m;
...
ocean_horizon_lod_levels_for_extent(const OceanSurfaceConfig& config, float mesh_half_extent_m) {
...
[[nodiscard]] inline OceanSurfaceConfig ocean_horizon_effective_mesh_config(const OceanSurfaceConfig& config,
                                                                     float camera_y_m,
                                                                     float water_datum_y_m,
                                                                     float planet_radius_m) {
    validate_ocean_config(config);
    ...
    OceanSurfaceConfig best_mesh_config = config;
    ...
    for (std::uint32_t lod_levels = min_lod_levels; lod_levels <= kOceanMaxMeshLodLevels;
         ++lod_levels) {
        OceanSurfaceConfig candidate = config;
