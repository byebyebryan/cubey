...
This note records the current terrain reboot capture set after the revision 8
...
promotes selected support cells as softer channel coverage. The
...
patch as the whole watershed. Revision `8` routes accumulation with continuous
...
terminate at or near an existing active channel, and softly paints selected
support so the review capture has a meaningful river footprint. The stress
recipe renders multiple selected corridors, so it is intentionally better for
...
drainage systems, and promoted support can expose fan-like bands where the
stream-order source is too broad. The next river-quality pass should evaluate
