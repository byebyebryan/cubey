...
next shared foundation work. The removed terrain lab and coastal demo remain
useful archived evidence, and the planned terrain reboot is still expected
...
shader golden tests beyond the initial FastNoiseLite samples, and volume/source
recipes remain later follow-ups once projects start consuming the seed, domain,
