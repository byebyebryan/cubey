...
Status: historical bakeoff with the accepted production result noted below.
...
The closed bakeoff compared `cached` and `planar` as production paths and used
`current-view` only as a historical reference. Planar is the default because:
...
The current runtime exposes only `cached` and `planar`. The canonical evidence
pack is generated by `projects/ocean/capture_ocean_review.sh`; the old focused
bakeoff sheets remain in archived outputs and git history.
