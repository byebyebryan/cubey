...
namespace cubey {
	...
	void MarchingCubesDemo::Init() {
		...
		cubes_render_prog_ = ShaderManager::Get()->CreateProgram("marching_cubes_render.VS.GS.FS");
	...
	void MarchingCubesDemo::Render() {
		...
		glm::vec3 directional_light_dir_es = glm::vec3(glm::normalize(MainCamera::Get()->view_mat() * directional_light_dir));
		...
		cubes_render_prog_->SetUniform("u_normal_mat", MainCamera::Get()->CalculateNormalMat(glm::mat4()));
